...
def fromCoordinates(x, y):
    ...
    all_nodes = [[Node(traffic_signal_factory) for _ in range(y)] for _ in range(x)]
    for i in range(x):
        for j in range(y):
            if i + 1 < x:
                next_node_x = all_nodes[i + 1][j]
                next_street_x = Street("E", default_walktime, next_node_x)
                all_nodes[i][j].addStreet(next_street_x)
            if j + 1 < y:
                next_node_y = all_nodes[i][j + 1]
                next_street_y = Street("N", default_walktime, next_node_y)
